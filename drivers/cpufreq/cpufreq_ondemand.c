...
unsigned int cpu_debug = 0;
static unsigned int power_save_speed = 918000;
static int power_save_enable = 0;
...
int is_power_save_policy(void)
{
	return power_save_enable;
}
EXPORT_SYMBOL(is_power_save_policy);

unsigned int get_power_save_speed(void)
{
	return power_save_speed;
}
EXPORT_SYMBOL(get_power_save_speed);

static ssize_t show_power_save(struct kobject *a,
...
static ssize_t store_power_save(struct kobject *a, struct attribute *b,
				    const char *buf, size_t count)
{
	char value;
	sscanf(buf, "%c", &value);
	...
	case 'N':
		if (power_save_enable) {
			cdbg("Restore ondemand governor\n");
			power_save_enable = 0;
		}
		break;
#if 0
	...
	case 'Y':
		if (!is_in_power_save) {
			cdbg("Enable power save mode Y\n");
			/* enable user cap */
			pm_qos_update_request(&pm_cpu_req, (s32)1134000);
			is_in_power_save = 1;
		}
		break;
#endif
...
static ssize_t show_cpu_debug_enable(struct kobject *a,
		struct attribute *b, char *buf)
{
	return sprintf(buf, "%d\n", cpu_debug);
}

static ssize_t store_cpu_debug_enable(struct kobject *a, struct attribute *b,
				    const char *buf, size_t count)
{
	unsigned int value;
...
static ssize_t show_power_save_cpu_speed(struct kobject *a,
		struct attribute *b, char *buf)
{
	return sprintf(buf, "%d\n", power_save_speed);
}

static ssize_t store_power_save_cpu_speed(struct kobject *a, struct attribute *b,
				    const char *buf, size_t count)
{
	unsigned int value;

	sscanf(buf, "%u", &value);
	power_save_speed = value;
...
#endif
define_one_global_rw(power_save);
define_one_global_rw(cpu_debug_enable);
define_one_global_rw(power_save_cpu_speed);
...
#endif
	&power_save.attr,
	&cpu_debug_enable.attr,
	&power_save_cpu_speed.attr,
	NULL
};
...
static void dbs_refresh_callback(struct work_struct *unused)
{
	...
	if (power_save_enable) {
		power_save_enable = 0;
		cdbg("Restore ondemand governor by event\n");
		cpufreq_update_policies();
	}
