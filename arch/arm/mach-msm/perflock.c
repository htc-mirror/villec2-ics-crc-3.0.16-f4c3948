...
extern int is_power_save_policy(void);
extern unsigned int get_power_save_speed(void);
			       ...
			       unsigned long event, void *data)
{
	...
	if (event == CPUFREQ_NOTIFY) {
		...
		} else if (is_power_save_policy()){
			policy->max = get_power_save_speed();
			if (debug_mask & PERF_CPUFREQ_LOCK_DEBUG)
				pr_info("%s: cpufreq power save policy %d %d\n"
